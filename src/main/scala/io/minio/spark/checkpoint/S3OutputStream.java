...
public class S3OutputStream extends OutputStream {
  ...
  public void write(final byte[] byteArray, final int off, final int len) {
    ...
    int mlen = len;
    int size;
    while (mlen > (size = this.buf.length - position)) {
      System.arraycopy(byteArray, moff, this.buf, this.position, size);
      this.position += size;
      flushBufferAndRewind();
      moff += size;
      mlen -= size;
    }
    System.arraycopy(byteArray, moff, this.buf, this.position, mlen);
    this.position += mlen;
  }
